...
function New-MmenuCBuildxDockerfile {
  ...
  if([string]::IsNullOrWhiteSpace($LabelSourcePath)){ $LabelSourcePath = $SourcePath }
  $dockerfile = @(
    'FROM scratch',
    'COPY . /home/',
    'WORKDIR /home',
    ('LABEL backup.source.path={0}' -f (Convert-MmenuCDockerfileQuoted $LabelSourcePath)),
...
function Invoke-MmenuCDockerfileBuildxPushUntilSuccess {
  ...
  try {
    ...
    $pushExpectedSourcePath = if([string]::IsNullOrWhiteSpace($ExpectedSourcePath)){ $SourcePath } else { $ExpectedSourcePath }
    return (Invoke-MmenuCDirectDockerPushUntilSuccess -DockerExe $DockerExe -ImageRef $ImageRef -ExpectedSourcePath $pushExpectedSourcePath -ExpectedRepository $ExpectedRepository -ExpectedTagName $ExpectedTagName -ExpectedSourceBytes $EstimatedBytes -ExpectedSourceFiles $ExpectedSourceFiles -ExpectedSourceFingerprint $ExpectedSourceFingerprint)
  } finally {
...
$labelSourcePath = $projectPath
$labelSourceOverride = [string]$env:HERMES_MMENU_SOURCE_LABEL_PATH_OVERRIDE
if(-not [string]::IsNullOrWhiteSpace($labelSourceOverride)){
  try { $labelSourcePath = [IO.Path]::GetFullPath($labelSourceOverride).TrimEnd('\') } catch { throw "${EntryPoint}: invalid source-label path override: $labelSourceOverride" }
  $labelRoot = [IO.Path]::GetPathRoot($labelSourcePath).TrimEnd('\')
  if($labelSourcePath -eq $labelRoot -or $labelSourcePath.StartsWith('\\') -or
     [IO.Path]::GetFileName($labelSourcePath) -ine $folderName){
    throw "${EntryPoint}: source-label path override must be a local application folder named $folderName: $labelSourcePath"
  }
  Write-MmenuC "[$EntryPoint] Source label path: $labelSourcePath"
}
...
if($allowExactCachePush -and (Test-MmenuCLocalExactImage -DockerExe $dockerExe -ImageRef $imageRef -SourcePath $labelSourcePath -Repository $repository -TagName $tagName -SourceBytes ([int64]$stats.TotalBytes) -SourceFiles ([int64]$stats.Files) -SourceFingerprint ([string]$stats.Fingerprint))){
  Write-MmenuC ("[$EntryPoint][push] started immediately after exact local image cache hit; image={0}" -f $imageRef)
  $cachePushCode = Invoke-MmenuCDirectDockerPushUntilSuccess -DockerExe $dockerExe -ImageRef $imageRef -ExpectedSourcePath $labelSourcePath -ExpectedRepository $repository -ExpectedTagName $tagName -ExpectedSourceBytes ([int64]$stats.TotalBytes) -ExpectedSourceFiles ([int64]$stats.Files) -ExpectedSourceFingerprint ([string]$stats.Fingerprint)
...
if($allowRemoteMetadataCache -and (Test-MmenuCRemoteExactLabels -ImageRef $imageRef -SourcePath $labelSourcePath -Repository $repository -TagName $tagName -SourceBytes ([int64]$stats.TotalBytes) -SourceFiles ([int64]$stats.Files) -SourceFingerprint ([string]$stats.Fingerprint))){
...
if($useDockerfileBuildx){
  ...
  $fastIgnorePath = ''
  try {
    New-MmenuCBuildxDockerfile -DockerfilePath $fastDockerfilePath -DockerignorePath $fastIgnorePath -SourcePath $projectPath -LabelSourcePath $labelSourcePath -Repository $repository -TagName $tagName -SourceBytes ([int64]$stats.TotalBytes) -SourceFiles ([int64]$stats.Files) -SourceFingerprint ([string]$stats.Fingerprint)
...
try{
  ...
  $labels = [ordered]@{
    'backup.source.path' = [string]$labelSourcePath
  ...
  if($directRegistryPush){
    ...
    if(-not (Test-MmenuCRemoteExactLabels -ImageRef $imageRef -SourcePath $labelSourcePath -Repository $repository -TagName $tagName -SourceBytes ([int64]$plan.total) -SourceFiles ([int64]$plan.files) -SourceFingerprint ([string]$stats.Fingerprint))){
  ...
  $pcode=Invoke-MmenuCDirectDockerPushUntilSuccess -DockerExe $dockerExe -ImageRef $imageRef -ExpectedSourcePath $labelSourcePath -ExpectedRepository $repository -ExpectedTagName $tagName -ExpectedSourceBytes ([int64]$plan.total) -ExpectedSourceFiles ([int64]$plan.files) -ExpectedSourceFingerprint ([string]$stats.Fingerprint)

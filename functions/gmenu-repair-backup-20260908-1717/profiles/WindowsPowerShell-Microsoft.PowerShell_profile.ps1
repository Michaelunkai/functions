...
# The built-in alias is AllScope, so a fast-profile reload can leave a
# Set-Location alias shadowing the new global target in the current script
# scope. Replace both scopes explicitly; ordinary cd/path behavior remains
# unchanged while numeric DUSH navigation stays available after be.
...
Set-Alias -Name cd -Value Invoke-DushCd -Option AllScope -Force
